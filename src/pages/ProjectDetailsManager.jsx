...
import {
  getCurrentDbUser,
  getProjectDetailsForManager,
  updateProjectWithMembers,
...
const ProjectDetailsManager = () => {
  ...
  const roleQuery = useQuery({
    queryKey: ["projects-edit-role", user?.id],
    queryFn: () => getCurrentDbUser(user.id, client),
    enabled: isLoaded && !!user?.id,
  });

  const detailsQuery = useQuery({
    queryKey: ["project-details", user?.id, projectId],
    queryFn: () => getProjectDetailsForManager({ clerkId: user.id, projectId, client }),
    enabled: isLoaded && !!user?.id && !!projectId && roleQuery.data?.role === "manager",
  });
  ...
  useEffect(() => {
    ...
    reset({
      project_name: detailsQuery.data.project.project_name || "",
      description: detailsQuery.data.project.description || "",
      tech_stack: detailsQuery.data.project.tech_stack || "",
      start_date: toDateInputValue(detailsQuery.data.project.start_date),
      expected_completion: toDateInputValue(detailsQuery.data.project.expected_completion),
      status: detailsQuery.data.project.status ,
      assignees: detailsQuery.data.assignees || [],
    });
  }, [detailsQuery.data, reset]);

  const mutation = useMutation({
    mutationFn: async (values) => {
      return await updateProjectWithMembers({
        clerkId: user.id,
        projectId,
        project: {
    ...
    onSuccess: (result) => {
      if (!result.success) {
        ...
        toast.error(result.message || "Project update failed");
        return;
      }
      ...
      queryClient.invalidateQueries({ queryKey: ["project-details", user?.id, projectId] });
    },
    onError: (error) => {
  ...
  if (roleQuery.isLoading || detailsQuery.isLoading) {
    return <Loader title="Loading project" subtitle="Fetching project details" />;
  }
  ...
  if (detailsQuery.isError || !detailsQuery.data) {
    return (
      <div className="space-y-4">
        <p className="text-rose-600 font-semibold">Unable to load project details.</p>
        <Button variant="outline" onClick={() => navigate("/projects/manager")}>Back to Projects</Button>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Review and update this project with full assignee validation.</p>
        </div>
      ...
      <Card className="border border-slate-200">
        ...
        <CardContent>
          <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="space-y-2">
              <Label htmlFor="project_document">Replace Project Documentation (PDF)</Label>
              <Input
                id="project_document"
                type="file"
              ...
              />
              {detailsQuery.data?.project?.project_report ? (
                <a
                  href={detailsQuery.data.project.project_report}
                  target="_blank"
                  rel="noreferrer"
